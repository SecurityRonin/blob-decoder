...
`blob-decoder.txt` is the recorded public API surface of the published library
crate, generated by
[`cargo-public-api`](https://github.com/cargo-public-api/cargo-public-api). The
`.github/workflows/public-api.yml` job regenerates the surface on every PR and
...
change as *breaking* against the crates.io release, while this baseline catches
...
```sh
cargo public-api -p blob-decoder --all-features \
  --omit blanket-impls,auto-trait-impls,auto-derived-impls > public-api/blob-decoder.txt
```
